...
-- 1. Re-aplicação e Reforço da RLS de Unidades
...
-- 2. Re-aplicação e Reforço da RLS de Profissionais
...
USING (
  (deleted_at IS NULL) AND (
    ...
    OR (
      (
        (unidade_id IS NOT NULL AND public.user_has_unit(auth.uid(), unidade_id)) 
        OR (secretaria_id IS NOT NULL AND public.user_has_secretaria(auth.uid(), secretaria_id))
      ) 
