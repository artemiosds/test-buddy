...
BEGIN
    ...
    -- MASTER e GESTOR (quando na sua secretaria) podem salvar sem restrição de unidade_id
    -- MASTER: acesso total
    ...
    IF NOT public.is_master(auth.uid()) THEN
        ...
        IF v_unidade_id IS NOT NULL THEN
            IF NOT (public.user_has_unit(auth.uid(), v_unidade_id) OR public.user_has_secretaria(auth.uid(), v_secretaria_id)) THEN
    ...
    ON CONFLICT (id) DO UPDATE SET
        ...
        updated_at = now()
    RETURNING jsonb_build_object('id', id, 'nome', nome_completo) INTO v_result;
