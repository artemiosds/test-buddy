...
CREATE OR REPLACE FUNCTION public.get_dashboard_summary(p_competencia_id uuid, p_unidade_id uuid DEFAULT NULL) 
RETURNS jsonb 
LANGUAGE plpgsql 
STABLE 
SECURITY DEFINER 
...
BEGIN    
    WITH filtered_freq AS (        
        SELECT             
            f.id,            
            CASE 
                WHEN LOWER(TRIM(f.status)) IN ('aprovada', 'aprovado', 'aprovadas') THEN 'aprovadas'
                WHEN LOWER(TRIM(f.status)) IN ('em_analise', 'em analise', 'em análise', 'enviada', 'enviadas') THEN 'em_analise'
                WHEN LOWER(TRIM(f.status)) IN ('rascunho', 'rascunhos', 'pendente', 'pendentes') THEN 'rascunho'
                WHEN LOWER(TRIM(f.status)) IN ('rejeitada', 'rejeitado', 'rejeitadas') THEN 'rejeitadas'
                ELSE LOWER(TRIM(f.status))
    ...
    vinculos AS (        
        SELECT v.nome, count(p.id) AS total        
        FROM profissionais p        
        JOIN vinculos v ON p.vinculo_id = v.id        
    ...
    SELECT jsonb_build_object(        
        'status_breakdown', COALESCE((SELECT jsonb_object_agg(normalized_status, total) FROM status_counts), '{}'::jsonb),        
