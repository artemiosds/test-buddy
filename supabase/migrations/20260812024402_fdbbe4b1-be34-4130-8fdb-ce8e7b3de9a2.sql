...
BEGIN
  ...
  matches AS (
    SELECT
      ...
      a.titular_nome,
      a.titular_cargo,
      -- caminho real da imagem: usa o da assinatura ativa; se ela nao tiver
      -- arquivo (assinatura eletronica), cai para a imagem mais recente do mesmo titular
      COALESCE(
  ...
  SELECT DISTINCT ON (m.perfil_codigo, m.ordem)
    ...
    m.escopo
  FROM matches m
  ORDER BY m.perfil_codigo, m.ordem, m.prioridade_escopo, m.prioridade_visual;
END;
$function$;