-- 1. Criar função is_master_db centralizada
...
BEGIN
  ...
  RETURN EXISTS (
    ...
    WHERE u.id = _user_id 
      ...
      AND (
        p.codigo = 'MASTER' OR 
        (u.acesso_todas_secretarias = true AND u.acesso_todas_unidades = true)
      )
  );
END $$;

-- 2. Atualizar is_master para usar a nova fonte de verdade (Removendo bypass por e-mail)
...
AS $$
BEGIN
  RETURN public.is_master_db(_user_id);
END $$;

-- 3. Atualizar is_master_core para usar a nova fonte de verdade
...
-- 4. Corrigir save_profissional_complete (Removendo bypass por e-mail e usando is_master_db)
...
BEGIN
    -- Identificação oficial de MASTER (Sem Cache, Sem E-mail hardcoded)
    ...
    IF NOT COALESCE(v_is_master, false) THEN
        -- Se não for MASTER, validar permissão funcional específica primeiro
        ...
        -- Validar escopo (Secretaria/Unidade)
    ...
    VALUES (
        ...
        (p_payload->>'carga_horaria_semanal')::INTEGER,
        COALESCE(p_payload->>'status', 'ativo')::public.profissional_status,
        ...
        p_payload->>'situacao_funcional',
        (p_payload->>'situacao_data_inicio')::DATE,
        (p_payload->>'situacao_data_fim')::DATE,
        p_payload->>'foto_url',
        (p_payload->>'salario_base')::NUMERIC,
        (p_payload->>'salario_liquido')::NUMERIC,
        (p_payload->>'horas_extras')::NUMERIC,
        (p_payload->>'adicional_noturno')::NUMERIC,
        (p_payload->>'salario_bruto')::NUMERIC,
        (p_payload->>'gratificacao_incentivo')::NUMERIC,
        (p_payload->>'vencimento_liquido')::NUMERIC,
        now(),
        now(),
        v_caller_id,
        v_caller_id
...
GRANT EXECUTE ON FUNCTION public.is_master_core(uuid) TO authenticated;
