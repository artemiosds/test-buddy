...
BEGIN
    ...
    -- Situação institucional única: prefere situacao_funcional e cai para status
    -- (836 cadastros têm situacao_funcional vazio e o status é a única fonte).
    prof_situacao AS (
        SELECT COALESCE(
                 NULLIF(LOWER(TRIM(situacao_funcional::text)), ''),
                 NULLIF(LOWER(TRIM(status::text)), ''),
                 'ativo'
               ) AS status_norm
        FROM profissionais
    ...
    status_prof AS (
        SELECT status_norm, count(*) AS total
        FROM prof_situacao
        GROUP BY 1
    ),
    kpis_situacao AS (
        SELECT
            count(*) AS total,
            count(*) FILTER (WHERE status_norm IN ('ativo', 'ferias', 'licenca_premio')) AS ativos,
            count(*) FILTER (WHERE status_norm = 'ativo') AS disponiveis,
            count(*) FILTER (WHERE status_norm IN ('ferias', 'licenca_premio')) AS ferias_licenca_premio,
            count(*) FILTER (WHERE status_norm IN (
                'afastado', 'afastado_laudo', 'atestado', 'afastamento_inss',
                'licenca', 'licenca_sem_vencimento', 'licenca_maternidade',
                'licenca_saude', 'licenca_luto', 'licenca_estudo',
                'falta_pad', 'vacancia', 'cedido'
            )) AS afastados,
            count(*) FILTER (WHERE status_norm IN ('desligado', 'inativo')) AS desligados
        FROM prof_situacao
    )
    SELECT jsonb_build_object(
        ...
        'kpis_situacao', (
            SELECT jsonb_build_object(
                ...
                'ferias_licenca_premio', ferias_licenca_premio,
                'afastados', afastados,
