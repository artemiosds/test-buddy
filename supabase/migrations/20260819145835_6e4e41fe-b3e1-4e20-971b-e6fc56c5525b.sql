...
-- 3. RLS de Unidades - Permissiva para Master
...
-- 5. RLS de Competência Unidades (A chave para listar unidades na folha)
DROP POLICY IF EXISTS "competencia_unidades_select" ON public.competencia_unidades;
CREATE POLICY "competencia_unidades_select" ON public.competencia_unidades
FOR SELECT
TO authenticated
USING (
  deleted_at IS NULL
...
-- 7. Garantir permissões de leitura
