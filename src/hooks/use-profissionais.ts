  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      let query = supabase
        .from("profissionais")
        .select(
          "id, nome_completo, cpf, matricula, unidade_id, setor_id, cargo_id, funcao_id, vinculo_id, status",
          { count: "exact" },
        );
      ...
      const mapped = rows.map((r: any) => ({
        ...r,
        unidade_nome: r.unidade_id ? (unidadesMap[r.unidade_id] ?? r.unidade_id) : null,
      }));
