...
export async function lerCadeiaProvedores(supabase: Rpc): Promise<CadeiaProvedores> {
  const { data, error } = await supabase.rpc("piso_ia_cadeia");
  if (error) throw new Error(error.message);
  const j = (data ?? {}) as CadeiaProvedores;
  return {
...
export const listarProvedoresIA = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data, error } = await (context.supabase as unknown as Rpc).rpc(
      "piso_ia_provedores_listar",
    );
    if (error) throw new Error(error.message);
    const j = (data ?? {}) as { provedores?: ProvedorPublico[]; config?: IaConfig };
    return {
      provedores: (j.provedores ?? []).sort((a, b) => a.prioridade - b.prioridade),
      config: j.config ?? { modo: "automatico" as const, provedor_id: null },
...
export const salvarProvedorIA = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d: unknown) => ProvedorInput.parse(d))
  .handler(async ({ data, context }) => {
    await ensurePermission(context.supabase, context.userId, "configuracao.editar");
    const { data: id, error } = await (context.supabase as unknown as Rpc).rpc(
...
export const excluirProvedorIA = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    await ensurePermission(context.supabase, context.userId, "configuracao.editar");
    const { error } = await (context.supabase as unknown as Rpc).rpc("piso_ia_provedor_excluir", {
      _id: data.id,
    });
    if (error) throw new Error(error.message);
    return { ok: true };
...
export const ordenarProvedoresIA = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    await ensurePermission(context.supabase, context.userId, "configuracao.editar");
    const { error } = await (context.supabase as unknown as Rpc).rpc("piso_ia_provedores_ordenar", {
      _ids: data.ids,
    });
    if (error) throw new Error(error.message);
    return { ok: true };
...
export const salvarModoIA = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    await ensurePermission(context.supabase, context.userId, "configuracao.editar");
    const { error } = await (context.supabase as unknown as Rpc).rpc("piso_ia_config_salvar", {
      _modo: data.modo,
...
export const testarProvedorIA = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (!chave && data.id) {
      const { data: k } = await (context.supabase as unknown as Rpc).rpc("piso_ia_provedor_key", {
        _id: data.id,
      });
    ...
    const provider = criarProvider({
      id: data.id ?? "novo",
      tipo: data.tipo as TipoProvedor,
      nome: data.nome,
      modelo: data.modelo,
    ...
    if (data.id) {
      await (context.supabase as unknown as Rpc)
