...
async function nomeUnidade(ctx, termo) {
    if (!termo)
        return null;
...
const listarProfissionais = {
    ...
    descricao: "Lista profissionais do cadastro com filtros opcionais (status, unidade, cargo/função, busca por nome ou CPF).",
    permissao: "profissional.visualizar",
    mutacao: false,
    argumentos: '{ status?: string (ativo|licenca|ferias|afastado|desligado|...), unidade?: string (nome), cargo?: string (nome do cargo ou função), busca?: string, limite?: number }',
    schema: z.object({
    ...
    async executar(a, ctx) {
        ...
        let q = ctx.supabase
            .from("profissionais")
            .select("id, nome_completo, cpf, matricula, status, situacao_funcional, carga_horaria_semanal, unidade_id, cargo_id, funcao_id", { count: "exact" })
            .is("deleted_at", null)
            .order("nome_completo")
            .limit(a.limite);
        if (a.status)
        ...
        if (a.busca)
            q = q.or(`nome_completo.ilike.%${a.busca}%,cpf.ilike.%${a.busca}%`);
        const unidades = ctx.unidadeId ? [ctx.unidadeId] : await nomeUnidade(ctx, a.unidade);
        if (unidades)
            q = q.in("unidade_id", unidades);
        ...
        const mapa = (r) => new Map((r.data ?? []).map((x) => [x.id, x.nome]));
        const mu = mapa(uni);
        const mc = mapa(car);
        const mf = mapa(fun);
        linhas = linhas.map((p) => ({
        ...
        if (a.cargo) {
            ...
            linhas = linhas.filter((l) => (l.cargo ?? "").toLowerCase().includes(alvo) ||
...
const contarProfissionais = {
    ...
    descricao: "Conta profissionais agrupando por status, unidade, cargo, função, vínculo ou setor. Ideal para perguntas do tipo 'quantos...' e para gráficos.",
    permissao: "profissional.visualizar",
    mutacao: false,
    ...
    async executar(a, ctx) {
        ...
        if (a.status)
            q = q.eq("status", a.status);
        if (ctx.unidadeId)
            q = q.eq("unidade_id", ctx.unidadeId);
        ...
        if (a.agrupar_por !== "status") {
            ...
            const m = new Map((refs ?? []).map((x) => [x.id, x.nome]));
            chave = (r) => (r[coluna] ? (m.get(r[coluna]) ?? "—") : "Não informado");
        }
...
const buscarProfissional = {
    ...
    async executar(a, ctx) {
        ...
        let q = ctx.supabase
            .from("profissionais")
            .select("id, nome_completo, cpf, matricula, status, situacao_funcional, data_admissao, carga_horaria_semanal, email, telefone, unidade_id, setor_id, cargo_id, funcao_id, vinculo_id")
            .is("deleted_at", null)
            .or(`nome_completo.ilike.%${a.termo}%,cpf.ilike.%${a.termo}%,matricula.ilike.%${a.termo}%`)
            .limit(10);
        if (ctx.unidadeId)
            q = q.eq("unidade_id", ctx.unidadeId);
        const { data, error } = await q;
...
const listarUnidades = {
    ...
    async executar(a, ctx) {
        ...
        let q = ctx.supabase
            ...
            .limit(a.limite);
        if (a.busca)
            q = q.ilike("nome", `%${a.busca}%`);
        if (ctx.unidadeId)
            q = q.eq("id", ctx.unidadeId);
        const { data, error } = await q;
        erro(error);
...
const listarFrequencias = {
    nome: "listarFrequencias",
    descricao: "Lista o andamento das folhas de frequência por unidade/competência (status, totais, datas de envio e aprovação).",
    ...
    async executar(a, ctx) {
        ...
        let q = ctx.supabase
            .from("frequencias")
            .select("tipo, status, total_profissionais, total_faltas, total_horas_extras, data_envio, data_aprovacao")
            .is("deleted_at", null)
...
const listarPendencias = {
    ...
    async executar(a, ctx) {
        ...
        if (a.status)
            q = q.eq("status", a.status);
        if (a.prioridade)
            q = q.eq("prioridade", a.prioridade);
        if (ctx.unidadeId)
            q = q.eq("unidade_id", ctx.unidadeId);
        const { data, error } = await q;
        erro(error);
...
const resumoPisoEnfermagem = {
    nome: "resumoPisoEnfermagem",
    descricao: "Resumo consolidado do Piso Nacional da Enfermagem por competência: quantidade por categoria, unidade e divergências.",
    ...
    async executar(a, ctx) {
        ...
        let q = ctx.supabase
            ...
            .limit(20000);
        if (a.competencia)
            q = q.eq("competencia", a.competencia);
        if (ctx.unidadeId)
            q = q.eq("unidade_id", ctx.unidadeId);
        const { data, error } = await q;
        erro(error);
...
export function ferramenta(nome) {
    return INDICE.get(nome) ?? null;
}
function itemCatalogo(f) {
...
export function catalogoParaPrompt(permitidas) {
    return permitidas
        .map((f) => {
...
export async function ferramentasPermitidas(ctx) {
    const { data } = await ctx.supabase.rpc("get_my_permissions");
    const codigos = new Set((data ?? []).map(String));
 ...
 */
export function filtrarPorAgente(ferramentas, agente) {
    if (!agente || agente === "geral")
        return ferramentas;
