 ...
 */
import { criarProvider, ProviderError } from "../ai-providers/runtime.server";
import { lerCadeiaProvedores } from "../piso-ia-provedores.functions";
/** Peso de afinidade modelo × intenção (heurística de roteamento). */
function afinidade(modelo, intencao) {
    const m = modelo.toLowerCase();
    const tem = (...t) => t.some((x) => m.includes(x));
    switch (intencao) {
        case "tecnico":
...
export function classificarIntencao(pergunta) {
    ...
    if (/\b(sql|consulta|código|codigo|script|erro|bug|função|api|json)\b/.test(p))
        return "tecnico";
...
export async function montarCadeia(supabase, intencao) {
    const cadeia = await lerCadeiaProvedores(supabase);
    ...
    if (cfgs.length === 0) {
        const modelo = intencao === "rapido" || intencao === "documento"
            ? "openai/gpt-5.6-luna"
    ...
    const ordenados = cadeia.modo === "manual"
        ? cfgs
        : [...cfgs].sort((a, b) => afinidade(b.modelo, intencao) - afinidade(a.modelo, intencao) ||
...
export async function conversar(opcoes) {
    ...
    if (cadeia.length === 0) {
        throw new Error("Nenhum provedor de IA disponível. Cadastre um em Piso da Enfermagem › Motor de Extração › Gerenciador de Provedores de IA.");
    }
    const tentativas = [];
    ...
    throw new Error(`Todos os provedores de IA falharam. ${tentativas.map((t) => t.erro).join(" | ")}`);
}
