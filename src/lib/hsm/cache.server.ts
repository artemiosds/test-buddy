...
const armazem = new Map<string, CacheEntrada>();
  ...
  if (valor && typeof valor === "object") {
    return Object.keys(valor as Record<string, unknown>)
      ...
      .reduce<Record<string, unknown>>((acc, k) => {
        ...
        if (v !== undefined && v !== null && v !== "") acc[k] = estavel(v);
        return acc;
      }, {});
  }
  return valor;
...
export function chaveCache(p: {
  ...
  ferramenta: string;
  argumentos: unknown;
  contexto?: { competencia?: string | null; unidade?: string | null } | null;
}): string {
...
export function lerCache(chave: string): CacheEntrada | null {
 ...
 */
export function invalidarCache(userId: string, prefixoFerramenta?: string): number {
  let n = 0;
