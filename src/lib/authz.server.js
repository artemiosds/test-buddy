...
// -----------------------------------------------------------------------------
const MFA_MSG = "Acesso bloqueado: seu perfil exige verificação em duas etapas (2FA). " +
...
async function mfaPendente(supabase, userId) {
    try {
...
export async function ensurePermission(supabase, userId, codigo, extra) {
    ...
    if (!data) {
        if (await mfaPendente(supabase, userId))
            throw new Error(MFA_MSG);
        throw new Error(`Acesso negado: permissão ${codigo} necessária.`);
    }
}
export async function ensureMaster(supabase, userId) {
 ...
 */
export async function emitEvento(supabase, tipo, agregado, agregado_id, dados = {}, opts = {}) {
    try {
        ...
        logger.warn("emit_evento.exception", { tipo, message: e.message });
        return null;
    }
}
/**
...
function classifyError(err) {
    ...
    if (/valida|zod|invalid input|required/i.test(m))
        return EVENTOS_TECNICOS.VALIDATION_ERROR;
    if (/upload|storage|bucket/i.test(m))
        return EVENTOS_TECNICOS.UPLOAD_ERROR;
    if (/duplicate key|foreign key|constraint|sql/i.test(m))
        return EVENTOS_TECNICOS.DATABASE_ERROR;
 ...
 */
export function withObservability(operacao, handler) {
    return async (args) => {
        try {
