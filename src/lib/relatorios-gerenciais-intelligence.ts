...
export async function getGerencialAggregate(): Promise<GerencialAggregate> {
  ...
  for (const p of profs) {
    ...
    if (p.unidade_id && p.status === "ativo")
      profAtivoPorUnidade.set(p.unidade_id, (profAtivoPorUnidade.get(p.unidade_id) ?? 0) + 1);
    if (p.unidade_id && p.status === "afastado")
      profAfastPorUnidade.set(p.unidade_id, (profAfastPorUnidade.get(p.unidade_id) ?? 0) + 1);
    if (p.unidade_id && p.status === "ferias")
      profFeriasPorUnidade.set(p.unidade_id, (profFeriasPorUnidade.get(p.unidade_id) ?? 0) + 1);
  ...
  // Distribuições
  const porUnidade = group(profs, (p) =>
  ...
  const porSecretaria = group(profs, (p) =>
    p.secretaria_id ? (mSec[p.secretaria_id] ?? "—") : "—",
  );
  const porTipoUnidade = group(unidades, (u) => u.tipo_unidade ?? "Sem tipo").map((r) => ({
    nome: r.nome,
    qtd: r.qtd,
  }));
  const porPorte = (() => {
  ...
  const distribuicoes = {
    porVinculo,
    porStatus: Object.entries(status)
      .map(([nome, qtd]) => ({ nome, qtd }))
      .sort((a, b) => b.qtd - a.qtd),
  ...
  const metricas: QualityMetric[] = [
    {
      chave: "cpf",
      rotulo: "CPF preenchido",
      total,
      ok: total - pendencias.semCpf,
      percentual: pct(total - pendencias.semCpf, total),
    },
    {
      chave: "matricula",
      rotulo: "Matrícula preenchida",
      total,
      ok: total - pendencias.semMatricula,
      percentual: pct(total - pendencias.semMatricula, total),
    },
    {
      chave: "telefone",
      rotulo: "Telefone preenchido",
      total,
      ok: total - pendencias.semTelefone,
      percentual: pct(total - pendencias.semTelefone, total),
    },
    {
      chave: "email",
      rotulo: "E-mail preenchido",
      total,
      ok: total - pendencias.semEmail,
      percentual: pct(total - pendencias.semEmail, total),
    },
    {
      chave: "nascimento",
      rotulo: "Data de nascimento",
      total,
      ok: total - pendencias.semNascimento,
      percentual: pct(total - pendencias.semNascimento, total),
    },
    {
      chave: "carga",
      rotulo: "Carga horária",
      total,
      ok: total - pendencias.semCargaHoraria,
      percentual: pct(total - pendencias.semCargaHoraria, total),
    },
    {
      chave: "unidade",
      rotulo: "Lotação (unidade)",
      total,
      ok: total - pendencias.semUnidade,
  ...
  const estruturaOrganizacional = pct(
    setores.length - setoresPend.semProfissionais,
    Math.max(1, setores.length),
  );
  const geralQ =
    Math.round(
      ((integridadeCadastral + lotacao + coberturaResponsaveis + estruturaOrganizacional) / 4) * 10,
    ) / 10;
  ...
  const comparativos: Comparativo[] = [
    {
      chave: "audit-30d",
      rotulo: "Auditoria 30d vs 30d anteriores",
      atual: auditNowCount,
      anterior: auditPrevCount,
      delta: auditNowCount - auditPrevCount,
      deltaPct: auditPrevCount ? pct(auditNowCount - auditPrevCount, auditPrevCount) : null,
    },
    {
      chave: "ativos-vs-afast",
      rotulo: "Ativos vs Afastados",
      atual: status["ativo"] ?? 0,
      anterior: status["afastado"] ?? 0,
      delta: (status["ativo"] ?? 0) - (status["afastado"] ?? 0),
      deltaPct: null,
    },
    {
      chave: "u-com-vs-sem-diretor",
      rotulo: "Unidades com vs sem Diretor",
      atual: unidades.length - unidadesPend.semDiretor,
      anterior: unidadesPend.semDiretor,
      delta: unidades.length - unidadesPend.semDiretor - unidadesPend.semDiretor,
      deltaPct: null,
    },
    {
      chave: "s-com-vs-sem-coord",
      rotulo: "Setores com vs sem Coordenador",
      atual: setores.length - setoresPend.semCoordenador,
      anterior: setoresPend.semCoordenador,
      delta: setores.length - setoresPend.semCoordenador - setoresPend.semCoordenador,
      deltaPct: null,
    },
    {
      chave: "cargos-ocup-vs-vazio",
      rotulo: "Cargos ocupados vs vagos",
      atual:
        (cRes.data ?? []).length -
        (cRes.data ?? []).filter((c) => !porCargo.find((x) => x.nome === c.nome && x.qtd > 0))
          .length,
      anterior: (cRes.data ?? []).filter(
        (c) => !porCargo.find((x) => x.nome === c.nome && x.qtd > 0),
      ).length,
      delta: 0,
      deltaPct: null,
    },
  ...
  const level = (n: number, a: number, c: number): Semaforo =>
    n >= c ? "vermelho" : n >= a ? "amarelo" : "verde";
  semItens.push({
    chave: "diretor",
    rotulo: "Unidades sem Diretor",
    nivel: level(unidadesPend.semDiretor, 1, 5),
    motivo: `${unidadesPend.semDiretor} sem diretor`,
  });
  semItens.push({
    chave: "coord",
    rotulo: "Setores sem Coordenador",
    nivel: level(setoresPend.semCoordenador, 5, 20),
    motivo: `${setoresPend.semCoordenador} sem coordenador`,
  });
  semItens.push({
    chave: "lotacao",
    rotulo: "Profissionais sem lotação",
    nivel: level(pendencias.semUnidade, 5, 20),
    motivo: `${pendencias.semUnidade} sem unidade`,
  });
  semItens.push({
    chave: "cnes",
    rotulo: "Unidades sem CNES",
    nivel: level(unidadesPend.semCnes, 1, 5),
    motivo: `${unidadesPend.semCnes} sem CNES`,
  });
  semItens.push({
    chave: "cnpj",
    rotulo: "Unidades sem CNPJ",
    nivel: level(unidadesPend.semCnpj, 1, 5),
    motivo: `${unidadesPend.semCnpj} sem CNPJ`,
  });
  semItens.push({
    chave: "cad",
    rotulo: "Integridade cadastral",
    nivel: integridadeCadastral < 75 ? "vermelho" : integridadeCadastral < 90 ? "amarelo" : "verde",
    motivo: `${integridadeCadastral.toFixed(1)}%`,
  });
  ...
  if (unidadesPend.semDiretor)
    alertas.push({
      id: "u-diretor",
      gravidade: unidadesPend.semDiretor >= 5 ? "vermelho" : "amarelo",
      titulo: "Unidades sem Diretor",
      detalhe: `${unidadesPend.semDiretor} unidade(s) sem responsável.`,
      quantidade: unidadesPend.semDiretor,
    });
  if (setoresPend.semCoordenador)
    alertas.push({
      id: "s-coord",
      gravidade: setoresPend.semCoordenador >= 20 ? "vermelho" : "amarelo",
      titulo: "Setores sem Coordenador",
      detalhe: `${setoresPend.semCoordenador} setor(es) sem coordenador.`,
      quantidade: setoresPend.semCoordenador,
    });
  if (pendencias.semUnidade)
    alertas.push({
      id: "p-unidade",
      gravidade: pendencias.semUnidade >= 20 ? "vermelho" : "amarelo",
      titulo: "Profissionais sem Lotação",
      detalhe: `${pendencias.semUnidade} profissional(is) sem unidade.`,
      quantidade: pendencias.semUnidade,
    });
  if (pendencias.semCpf)
    alertas.push({
      id: "p-cpf",
      gravidade: "vermelho",
      titulo: "Cadastros sem CPF",
      detalhe: `${pendencias.semCpf} profissional(is) sem CPF.`,
      quantidade: pendencias.semCpf,
    });
  if (pendencias.semMatricula)
    alertas.push({
      id: "p-mat",
      gravidade: "amarelo",
      titulo: "Sem Matrícula",
      detalhe: `${pendencias.semMatricula} profissional(is) sem matrícula.`,
      quantidade: pendencias.semMatricula,
    });
  if (pendencias.semTelefone)
    alertas.push({
      id: "p-tel",
      gravidade: "amarelo",
      titulo: "Sem Telefone",
      detalhe: `${pendencias.semTelefone} profissional(is) sem telefone.`,
      quantidade: pendencias.semTelefone,
    });
  if (pendencias.semCargo)
    alertas.push({
      id: "p-cargo",
      gravidade: "amarelo",
      titulo: "Sem Cargo",
      detalhe: `${pendencias.semCargo} profissional(is) sem cargo.`,
      quantidade: pendencias.semCargo,
    });
  if (unidadesPend.semCnes)
    alertas.push({
      id: "u-cnes",
      gravidade: "vermelho",
      titulo: "Unidades sem CNES",
      detalhe: `${unidadesPend.semCnes} unidade(s) sem CNES.`,
      quantidade: unidadesPend.semCnes,
    });
  if (unidadesPend.semCnpj)
    alertas.push({
      id: "u-cnpj",
      gravidade: "amarelo",
      titulo: "Unidades sem CNPJ",
      detalhe: `${unidadesPend.semCnpj} unidade(s) sem CNPJ.`,
      quantidade: unidadesPend.semCnpj,
    });
  if (unidadesPend.semProfissionais)
    alertas.push({
      id: "u-vazias",
      gravidade: "amarelo",
      titulo: "Unidades sem Profissionais",
      detalhe: `${unidadesPend.semProfissionais} unidade(s) sem lotação.`,
      quantidade: unidadesPend.semProfissionais,
    });
  ...
  const resumoExecutivo: string[] = [
    ...
    topUn
      ? `Maior unidade: ${topUn.nome} com ${topUn.valor} profissional(is).`
      : "Sem unidades com lotação registrada.",
  ...
  // Auditoria
  const auditRows = (auditNow.data ?? []) as Array<{
    id: number;
    ocorrido_em: string;
    operacao: string;
    tabela: string;
