...
import { ACOES, EVENTOS, emitEvento, ensurePermission, withObservability } from "./authz.server";
...
export const getPendencia = createServerFn({ method: "GET" })
  ...
  .handler(
    withObservability("pendencia.detalhe", async ({ data, context }) => {
      const { supabase } = context;
...
export const criarPendencia = createServerFn({ method: "POST" })
  ...
  .handler(
    withObservability("pendencia.criar", async ({ data, context }) => {
      ...
      const { data: numero, error: numErr } = await supabase.rpc("proximo_numero_pendencia", {
        _secretaria_id: data.secretaria_id,
      });
...
export const atribuirPendencia = createServerFn({ method: "POST" })
  ...
  .handler(
    withObservability("pendencia.atribuir", async ({ data, context }) => {
      const { supabase, userId } = context;
...
export const responderPendencia = createServerFn({ method: "POST" })
  ...
  .handler(
    withObservability("pendencia.responder", async ({ data, context }) => {
      const { supabase, userId } = context;
      const p = await loadEscopo(supabase, data.id);
...
export const resolverPendencia = createServerFn({ method: "POST" })
  ...
  .handler(
    withObservability("pendencia.resolver", async ({ data, context }) => {
      const { supabase, userId } = context;
      const p = await loadEscopo(supabase, data.id);
...
export const reabrirPendencia = createServerFn({ method: "POST" })
  ...
  .handler(
    withObservability("pendencia.reabrir", async ({ data, context }) => {
      const { supabase, userId } = context;
      const p = await loadEscopo(supabase, data.id);
...
export const cancelarPendencia = createServerFn({ method: "POST" })
  ...
  .handler(
    withObservability("pendencia.cancelar", async ({ data, context }) => {
      const { supabase, userId } = context;
      const p = await loadEscopo(supabase, data.id);
      ...
      if (p.status === "resolvida")
        throw new Error("Pendência já resolvida não pode ser cancelada.");
...
export const alterarPrioridade = createServerFn({ method: "POST" })
  ...
  .handler(
    withObservability("pendencia.alterar_prioridade", async ({ data, context }) => {
      const { supabase, userId } = context;
      const p = await loadEscopo(supabase, data.id);
...
export const alterarPrazo = createServerFn({ method: "POST" })
  ...
  .handler(
    withObservability("pendencia.alterar_prazo", async ({ data, context }) => {
      const { supabase, userId } = context;
      const p = await loadEscopo(supabase, data.id);
      await ensurePermission(supabase, userId, ACOES.PENDENCIA_EDITAR, {
        _unidade_id: p.unidade_id,
      ...
      const { error } = await supabaseAdmin.from("pendencias").update(patch).eq("id", data.id);
      if (error) throw new Error(error.message);
