...
export const garantirLinhaFolha = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    // Usamos supabaseAdmin para garantir a criação da linha mesmo se o RLS do usuário estiver restritivo,
    // já que o controle de permissão é feito explicitamente via ensurePermission acima.
    ...
    try {
      ...
      console.error("[garantirLinhaFolha] Erro crítico:", err);
      // Retornamos um objeto de erro em vez de lançar exceção para evitar tela branca no React
      return { 
        success: false, 
        error: "Falha ao gerar linha de folha: " + (err.message || "Erro desconhecido"),
        id: null 
      } as any;
    }
  });
