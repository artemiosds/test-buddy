...
import {
  ArrowLeft, Users, Briefcase, ClipboardList, AlertCircle, Clock,
  RefreshCw, UserCheck, Layers, ArrowRightLeft, Search,
} from "lucide-react";
...
function SetorPainelPage() {
  ...
  const setorQ = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("setores")
        .select("id, nome, sigla, status, unidade_id, gestor:profissionais!setores_gestor_id_fkey(id, nome_completo), unidade:unidades(id, nome, sigla)")
        .eq("id", id).is("deleted_at", null).maybeSingle();
      if (error) throw error;
      return data as unknown as {
        id: string; nome: string; sigla: string | null; status: string; unidade_id: string;
  ...
  const profsQ = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("profissionais")
        .select("id, nome_completo, matricula, status, cargo_id, funcao_id, vinculo_id, cargo:cargos(nome), funcao:funcoes(nome), vinculo:vinculos(nome, natureza)")
        .eq("setor_id", id).is("deleted_at", null)
  ...
  const movsQ = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select("id, data_inicio, tipo_evento, setor_anterior_id, setor_novo_id, profissional:profissionais(nome_completo), setor_ant:setores!profissional_historico_funcional_setor_anterior_id_fkey(nome, sigla), setor_novo:setores!profissional_historico_funcional_setor_novo_id_fkey(nome, sigla)")
    ...
    <div className="space-y-6">
      <Breadcrumb>
        <BreadcrumbList>
          ...
          <BreadcrumbSeparator />
          <BreadcrumbItem>
            <BreadcrumbLink asChild><Link to="/setores">Setores</Link></BreadcrumbLink>
          </BreadcrumbItem>
          <BreadcrumbSeparator />
          <BreadcrumbItem><BreadcrumbPage>{s.nome}</BreadcrumbPage></BreadcrumbItem>
        </BreadcrumbList>
      </Breadcrumb>
      ...
      <PageHeader
        ...
        description={[
          ...
          compAtiva ? `Competência ${String(compAtiva.mes).padStart(2, "0")}/${compAtiva.ano}` : null,
        ].filter(Boolean).join(" • ")}
        actions={
          <>
      ...
      <Section title="Resumo">
        <KpiGrid>
          ...
          <KpiCard label="Unidade" value={s.unidade?.sigla ?? s.unidade?.nome ?? "—"} loading={false} />
        </KpiGrid>
      </Section>

      <Section title="Distribuição">
        <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
          <DistCard title="Profissionais por Cargo" rows={distCargo} loading={profsQ.isLoading} labelHeader="Cargo" />
          <DistCard title="Profissionais por Vínculo" rows={distVinculo} loading={profsQ.isLoading} labelHeader="Vínculo" />
          <DistCard title="Profissionais por Status" rows={distStatus} loading={profsQ.isLoading} labelHeader="Status" renderLabel={(v) => <StatusBadge domain="profissional" value={v} />} />
        </div>
      </Section>

      <Section title="Operação">
        <KpiGrid>
          <KpiCard label="Pendências abertas" value={a.pendencias.data ?? 0} loading={a.pendencias.isLoading} hint="Unidade-mãe" icon={<AlertCircle className="h-4 w-4" />} />
          <KpiCard label="Horas extras (competência)" value={a.totalHorasExtras.toLocaleString("pt-BR")} loading={a.frequencias.isLoading} hint="Unidade-mãe" icon={<Clock className="h-4 w-4" />} />
          <KpiCard label="Faltas (competência)" value={a.totalFaltas.toLocaleString("pt-BR")} loading={a.frequencias.isLoading} hint="Unidade-mãe" icon={<ClipboardList className="h-4 w-4" />} />
        </KpiGrid>
        <p className="text-xs text-muted-foreground">Frequências são agregadas por unidade+competência no schema atual e refletem o total da unidade-mãe.</p>
      </Section>
        ...
        <Card>
          <CardHeader className="flex flex-row flex-wrap items-center justify-between gap-2">
            <div className="flex flex-col gap-1">
              ...
              <div className="text-xs text-muted-foreground">
                Responsável: <span className="font-medium text-foreground">{s.gestor?.nome_completo ?? "—"}</span>
              </div>
            </div>
          ...
          <CardContent>
            <DataTable
              ...
              emptyDescription={teamSearch ? "Nenhum resultado para a busca." : "Nenhum profissional vinculado a este setor."}
            />
          </CardContent>
        </Card>
      </Section>
        ...
        <Card>
          ...
          <CardContent>
            {movsQ.isLoading ? (
              ...
              <EmptyState title="Nenhuma movimentação registrada" description="Não há histórico de lotação envolvendo este setor." />
            ) : (
                  ...
                  return (
                    <li key={row.id} className="flex flex-wrap items-center justify-between gap-2 py-2">
                      <div>
                        ...
                        <div className="text-xs text-muted-foreground">
                          {row.tipo_evento} • {(row.setor_ant?.sigla ?? row.setor_ant?.nome) ?? "—"} → {(row.setor_novo?.sigla ?? row.setor_novo?.nome) ?? "—"}
                        </div>
                      </div>
...
function DistCard({
  title, rows, loading, labelHeader = "Item", renderLabel,
}: { title: string; rows: Array<{ label: string; total: number }>; loading?: boolean; labelHeader?: string; renderLabel?: (v: string) => React.ReactNode }) {
  const cols: DataTableColumn<{ label: string; total: number }>[] = [
    { key: "label", header: labelHeader, cell: (r) => renderLabel ? renderLabel(r.label) : r.label },
