...
import {
  Section, ExecutiveSummary, SmartAlerts, QualityIndex, QualityBreakdown, SemaphoreCard,
  ChartCard, BarChartH, BarChartV, PieChartCard, LineChartCard, RadarQuality,
  ComparativeCard, RankingList, KpiGrid, type KpiSpec,
} from "./sections";
import type { GerencialAggregate } from "@/lib/relatorios-gerenciais-intelligence";

export type FocoGerencial = "geral" | "profissionais" | "unidades" | "setores" | "cargos" | "funcoes" | "estrutura" | "piso" | "auditoria";
...
function buildKpis(a: GerencialAggregate, foco: FocoGerencial): KpiSpec[] {
  ...
  if (foco === "profissionais") {
    return [
      ...
      { label: "Sem Matrícula", value: p.semMatricula, tone: p.semMatricula ? "warning" : "success" },
      ...
      { label: "Cadastro Completo", value: cad, tone: a.qualidade.integridadeCadastral >= 90 ? "success" : a.qualidade.integridadeCadastral >= 75 ? "warning" : "danger" },
    ];
  }
  if (foco === "unidades") {
    ...
    return [
      ...
      { label: "Sem Coordenador (setores)", value: st.semCoordenador, tone: st.semCoordenador ? "warning" : "success" },
      ...
      { label: "Sem Profissionais", value: u.semProfissionais, tone: u.semProfissionais ? "warning" : "success" },
  ...
  if (foco === "setores") {
    const media = t.setores ? Math.round(((t.profissionais - p.semSetor) / t.setores) * 10) / 10 : 0;
    ...
    return [
      ...
      { label: "Sem Coordenador", value: st.semCoordenador, tone: st.semCoordenador ? "warning" : "success" },
      { label: "Sem Profissionais", value: st.semProfissionais, tone: st.semProfissionais ? "warning" : "success" },
      ...
      { label: "Setores por Unidade (média)", value: t.unidades ? Math.round((t.setores / t.unidades) * 10) / 10 : 0 },
      { label: "Estrutura Organizacional", value: a.qualidade.estruturaOrganizacional + "%" },
      { label: "Profissionais sem Setor", value: p.semSetor, tone: p.semSetor ? "warning" : "success" },
  ...
  if (foco === "cargos") {
    ...
    const ocupados = a.distribuicoes.porCargo.filter((r) => r.nome !== "Sem cargo" && r.qtd > 0).length;
    return [
      ...
      { label: "Média por cargo", value: t.cargos ? Math.round((t.profissionais / t.cargos) * 10) / 10 : 0 },
      { label: "Profissionais sem cargo", value: p.semCargo, tone: p.semCargo ? "warning" : "success" },
      { label: "Concentração top-1 (%)", value: t.profissionais && top ? pct(top.valor, t.profissionais) + "%" : "—" },
      { label: "Top-5 concentração (%)", value: pct(a.rankings.cargosMaisUtilizados.slice(0, 5).reduce((s, r) => s + r.valor, 0), t.profissionais) + "%" },
      { label: "Cargos distintos ativos", value: ocupados },
      { label: "Ativos ocupando cargos", value: (s["ativo"] ?? 0) - p.semCargo > 0 ? (s["ativo"] ?? 0) - p.semCargo : (s["ativo"] ?? 0) },
  ...
  if (foco === "funcoes") {
    ...
    const ocupadas = a.distribuicoes.porFuncao.filter((r) => r.nome !== "Sem função" && r.qtd > 0).length;
    return [
      ...
      { label: "Ocupação do topo", value: top ? top.valor : 0 },
      { label: "Média por função", value: t.funcoes ? Math.round((t.profissionais / t.funcoes) * 10) / 10 : 0 },
      { label: "Sem função", value: p.semFuncao },
      { label: "Concentração top-1 (%)", value: t.profissionais && top ? pct(top.valor, t.profissionais) + "%" : "—" },
      { label: "Top-5 concentração (%)", value: pct(a.rankings.funcoesMaisUtilizadas.slice(0, 5).reduce((s, r) => s + r.valor, 0), t.profissionais) + "%" },
      { label: "Ativos com função", value: (s["ativo"] ?? 0) - p.semFuncao > 0 ? (s["ativo"] ?? 0) - p.semFuncao : (s["ativo"] ?? 0) },
      { label: "Cobertura Diretores", value: pct(t.unidades - u.semDiretor, t.unidades) + "%" },
      { label: "Cobertura Coord.", value: pct(t.setores - st.semCoordenador, t.setores) + "%" },
      { label: "Integridade Cadastral", value: cad },
      { label: "Qualidade Geral", value: a.qualidade.geral + "%" },
  ...
  if (foco === "estrutura") {
    return [
      ...
      { label: "Cobertura Diretores", value: pct(t.unidades - u.semDiretor, t.unidades) + "%", tone: "success" },
      { label: "Cobertura Coordenadores", value: pct(t.setores - st.semCoordenador, t.setores) + "%", tone: "success" },
      { label: "Unidades sem Diretor", value: u.semDiretor, tone: u.semDiretor ? "danger" : "success" },
      { label: "Setores sem Coordenador", value: st.semCoordenador, tone: st.semCoordenador ? "warning" : "success" },
      ...
      { label: "Setores por Unidade (média)", value: t.unidades ? Math.round((t.setores / t.unidades) * 10) / 10 : 0 },
      { label: "Profissionais por Setor (média)", value: t.setores ? Math.round(((t.profissionais - p.semSetor) / t.setores) * 10) / 10 : 0 },
      { label: "Estrutura Organizacional", value: a.qualidade.estruturaOrganizacional + "%" },
      { label: "Qualidade Geral", value: a.qualidade.geral + "%" },
    ];
  }
  if (foco === "auditoria") {
    ...
    return [
      ...
      { label: "Δ vs 30d anteriores", value: compAudit ? (compAudit.delta >= 0 ? "+" : "") + compAudit.delta : 0 },
      ...
      { label: "Tabela mais alterada", value: topTable ? `${topTable.nome} (${topTable.qtd})` : "—" },
      { label: "Média por dia", value: au.porDia.length ? Math.round((au.totalEventos / au.porDia.length) * 10) / 10 : 0 },
      { label: "Pico horário", value: (au.porHora.reduce((m, x) => (x.qtd > (m?.qtd ?? 0) ? x : m), au.porHora[0])?.hora ?? "—") },
      ...
      { label: "Exclusões", value: au.porOperacao.find((o) => o.nome === "delete")?.qtd ?? 0, tone: "danger" },
      { label: "Logins", value: au.porOperacao.find((o) => o.nome === "login")?.qtd ?? 0 },
      { label: "Ações customizadas", value: au.porOperacao.find((o) => o.nome === "custom")?.qtd ?? 0 },
    ];
  }
  // geral
  return [
...
export function IntelligencePanel({ foco = "geral", titulo }: { foco?: FocoGerencial; titulo?: string }) {
  ...
  if (isError) return <EmptyState title="Falha ao carregar inteligência" description={String((error as Error)?.message ?? "")} />;
    ...
    <div className="space-y-4">
      ...
      <Section title="Índice de Qualidade e Integridade">
        <div className="grid gap-3 lg:grid-cols-3">
          ...
          <ChartCard title="Radar da Qualidade" height={260}><RadarQuality q={a.qualidade} /></ChartCard>
        </div>
      ...
      <Section title="Gráficos">
        <div className="grid gap-3 lg:grid-cols-2">
          <ChartCard title="Profissionais por Unidade (top 20)" height={340}><BarChartH data={d.porUnidade} color="#3B82F6" height={340} /></ChartCard>
          <ChartCard title="Profissionais por Setor (top 20)" height={340}><BarChartH data={d.porSetor} color="#EC4899" height={340} /></ChartCard>
          <ChartCard title="Profissionais por Cargo (top 20)" height={340}><BarChartH data={d.porCargo} color="#F59E0B" height={340} /></ChartCard>
          <ChartCard title="Profissionais por Função (top 20)" height={340}><BarChartH data={d.porFuncao} color="#8B5CF6" height={340} /></ChartCard>
          <ChartCard title="Distribuição por Vínculo"><PieChartCard data={d.porVinculo} /></ChartCard>
          <ChartCard title="Distribuição por Sexo"><PieChartCard data={d.porSexo} /></ChartCard>
          <ChartCard title="Faixa Etária"><BarChartV data={d.porFaixaEtaria} color="#10B981" /></ChartCard>
          <ChartCard title="Status Funcional"><BarChartV data={d.porStatus} color="#6366F1" /></ChartCard>
          <ChartCard title="Tempo de Serviço"><BarChartV data={d.porTempoServico} color="#14B8A6" /></ChartCard>
          <ChartCard title="Distribuição por Secretaria"><PieChartCard data={d.porSecretaria} /></ChartCard>
          <ChartCard title="Unidades por Tipo"><PieChartCard data={d.porTipoUnidade} /></ChartCard>
          <ChartCard title="Unidades por Porte"><BarChartV data={d.porPorte} color="#F97316" /></ChartCard>
          {foco === "auditoria" && (
            <>
              <ChartCard title="Auditoria — operações por dia (30d)"><LineChartCard data={au.porDia as unknown as Record<string, string | number>[]} nameKey="dia" dataKey="qtd" color="#EF4444" /></ChartCard>
              <ChartCard title="Auditoria — horário de maior atividade"><BarChartV data={au.porHora.map((h) => ({ nome: h.hora, qtd: h.qtd }))} color="#EF4444" /></ChartCard>
            </>
          )}
        </div>
      </Section>
      ...
      <Section title="Rankings">
        <div className="grid gap-3 lg:grid-cols-2 xl:grid-cols-3">
          ...
          {foco === "auditoria" && (
            <RankingList titulo="Usuários mais ativos (auditoria 30d)" itens={au.porUsuario.map((u) => ({ nome: u.nome, valor: u.qtd }))} />
          )}
        </div>
      </Section>
    </div>
